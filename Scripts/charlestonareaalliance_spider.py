...
class CharlestonareaallianceSpiderSpider(scrapy.Spider):
    name = 'charlestonareaalliance_spider'
    start_urls = ['https://business.charlestonareaalliance.org/list']
    custom_settings = {
        'FEED_URI': 'charlestonareaalliance.csv',
        'FEED_FORMAT': 'csv',
    ...
    def detail_page(self, response):
        ...
        item['Social_Media'] = ', '.join(data.css('::attr(href)').get() for data in
                                         response.css('div[id="mn-memberinfo-block-socialnetworks"] li a'))
        item['Description'] = response.css('div[itemprop="description"] p::text').get('').strip()
        item['Source_URL'] = 'https://business.charlestonareaalliance.org/list'
        item['Occupation'] = response.css('ul.mn-member-cats li::text').get('').strip()
        item['Lead_Source'] = 'business.charlestonareaalliance'
