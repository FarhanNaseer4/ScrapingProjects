...
class MedcSpider(scrapy.Spider):
    name = 'medc'
    start_urls = ['https://members.medc.ms/list']
    ...
    def detail_page(self, response):
        try:
            item = response.meta['item']
            item['Business Name'] = response.css('h1.mb-3::text').get('').strip()
            ...
            item['Business_Site'] = response.css('li#mn-memberinfo-block-website a::attr(href)').get('').strip()
            item['Social_Media'] = ', '.join(
                data.css('::attr(href)').get('') for data in response.css('li.gz-card-social a'))
            item['Source_URL'] = 'https://members.medc.ms/list'
