...
class LawrencechamberSpiderSpider(scrapy.Spider):
    name = 'lawrencechamber_spider'
    start_urls = ['https://members.lawrencechamber.com/list']

    custom_settings = {
        'FEED_URI': 'lawrencechamber.csv',
        'FEED_FORMAT': 'csv',
    ...
    def detail_page(self, response):
        item = response.meta['item']
        item['Business Name'] = response.css('div.et_pb_text_inner h1::text').get('').strip()
        ...
        if fullname:
            ...
            if len(fullname.split(' ')) > 2:
                ...
                item['Last Name'] = fullname.split(' ')[-1].strip()
        item['Source_URL'] = 'https://members.lawrencechamber.com/list'
        item['Occupation'] = response.css('span.gz-cat::text').get('').strip()
        item['Lead_Source'] = 'members.lawrencechamber'
