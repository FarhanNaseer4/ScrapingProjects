...
class PerryiaSpiderSpider(scrapy.Spider):
    name = 'perryia_spider'
    start_urls = ['http://perryia.chambermaster.com/list/']

    custom_settings = {
        'FEED_URI': 'perryia_chambermaster.csv',
        'FEED_FORMAT': 'csv',
    ...
    def detail_page(self, response):
        item = dict()
        item['Business Name'] = response.css('div[id="banner-container"] h2.wsite-content-title::text').get('').strip()
        ...
        item['Source_URL'] = 'http://perryia.chambermaster.com/list/'
        item['Occupation'] = response.css('span.gz-cat::text').get('').strip()
        item['Lead_Source'] = 'perryia.chambermaster'
