...
class MarshallmochamberSpiderSpider(scrapy.Spider):
    name = 'marshallmochamber_spider'
    start_urls = ['http://business.marshallmochamber.com/members/']

    custom_settings = {
        'FEED_URI': 'marshallmochamber.csv',
        'FEED_FORMAT': 'csv',
    ...
    def parse(self, response):
        for data in response.css('div[id="mn-alphanumeric"] a'):
            ...
            yield scrapy.Request(url=url, callback=self.parse_listing, headers=self.headers, meta={'item': item})

    def parse_listing(self, response):
        item = response.meta['item']
        for data in response.css('div[itemprop="name"] a'):
            detail_url = data.css('::attr(href)').get()
            yield scrapy.Request(url=detail_url, callback=self.detail_page, headers=self.headers, meta={'item': item})
    ...
    def detail_page(self, response):
        ...
        item['Social_Media'] = ', '.join(data.css('::attr(href)').get() for data in response.css('div[id="mn-memberinfo-block-socialnetworks"] li a'))
        item['Source_URL'] = 'http://business.marshallmochamber.com/members/'
        item['Occupation'] = response.css('ul.mn-member-cats li::text').get('').strip()
        item['Lead_Source'] = 'marshallmochamber'
