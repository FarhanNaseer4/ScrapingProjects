...
class ArdmoreSpiderSpider(scrapy.Spider):
    name = 'ardmore_spider'
    start_urls = ['https://business.ardmore.org/list/']

    custom_settings = {
        'FEED_URI': 'ardmore.csv',
        'FEED_FORMAT': 'csv',
    ...
    def detail_page(self, response):
        item = dict()
        item['Business Name'] = response.css('h1.pageTitle::text').get('').strip()
        ...
        item['Social_Media'] = ', '.join(
            data.css('::attr(href)').get() for data in response.css('ul.mn-details-share li a'))
        item['Source_URL'] = 'https://business.ardmore.org/list/'
        item['Occupation'] = response.css('ul.mn-member-cats li::text').get('').strip()
        item['Lead_Source'] = 'business.ardmore'
