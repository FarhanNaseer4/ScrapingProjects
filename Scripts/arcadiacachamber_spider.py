...
class ArcadiacachamberSpiderSpider(scrapy.Spider):
    name = 'arcadiacachamber_spider'
    start_urls = ['https://web.arcadiacachamber.org/allcategories']

    base_url = 'https://web.arcadiacachamber.org{}'
    custom_settings = {
        'FEED_URI': 'arcadiacachamber.csv',
        'FEED_FORMAT': 'csv',
    ...
    def listing_page(self, response):
        for data in response.css('div.ListingResults_All_CONTAINER'):
            ...
            item['Category'] = response.css('h1.wideWLTitle::text').get('').strip()
            item['Source_URL'] = 'https://web.arcadiacachamber.org/allcategories'
            item['Meta_Description'] = 'Being a Member of the Arcadia Chamber of Commerce has rewarding benefits that ' \
                                       'can help you grow your business, build relationships and network. Join today! '
            item['Occupation'] = response.css('h1[id="wc-pageTitle"]::text').get('').strip()
            item['Lead_Source'] = 'web.arcadiacachamber'
