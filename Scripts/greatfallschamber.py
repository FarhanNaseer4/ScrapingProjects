...
class GreatfallschamberSpider(scrapy.Spider):
    name = 'greatfallschamber'
    start_urls = ['https://members.greatfallschamber.org/list/']

    custom_settings = {
        'FEED_URI': 'greatfallschamber.csv',
        'FEED_FORMAT': 'csv',
    ...
    def detail_page(self, response):
        try:
            ...
            item['Business_Site'] = response.css('li[id="mn-memberinfo-block-website"] a::attr(href)').get('').strip()
            item['Social_Media'] = ', '.join(
                data.css('::attr(href)').get('') for data in response.css('div[id="mn-memberinfo-block-socialnetworks"] a'))
            item['Source_URL'] = 'https://members.greatfallschamber.org/list/'
            item['Occupation'] = response.css('ul.mn-member-cats li::text').get('').strip()
            item['Lead_Source'] = 'greatfallschamber'
