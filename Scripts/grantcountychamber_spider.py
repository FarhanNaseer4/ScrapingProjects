...
class GrantcountychamberSpiderSpider(scrapy.Spider):
    name = 'grantcountychamber_spider'
    start_urls = ['https://www.grantcountychamber.com/list']

    custom_settings = {
        'FEED_URI': 'grantcountychamber.csv',
        'FEED_FORMAT': 'csv',
    ...
    def parse_listing(self, response):
        for data in response.css('div.mn-listingcontent div[itemprop="name"] a'):
    ...
    def detail_page(self, response):
        ...
        item['Social_Media'] = ', '.join(data.css('::attr(href)').get() for data in response.css('ul.mn-details-share li a'))
        item['Source_URL'] = 'https://www.grantcountychamber.com/list'
        item['Occupation'] = response.css('ul.mn-member-cats li::text').get('').strip()
        item['Lead_Source'] = 'grantcountychamber'
