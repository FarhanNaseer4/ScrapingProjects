...
class DoctorSpiderSpider(scrapy.Spider):
    name = 'doctor_spider'
    start_urls = ['https://www.healthcare4ppl.com/physician']
    base_url = 'https://www.healthcare4ppl.com{}'
    custom_settings = {
        'FEED_URI': 'healthcare_doctor.csv',
        'FEED_FORMAT': 'csv',
    ...
    def get_listing_page(self, response):
        for data in response.css('div.h4 a'):
    ...
    def get_detail_page(self, response):
        item = dict()
        fullname = response.css('div.col-md-9 h1::text').get('').strip()
        ...
        item['Source_URL'] = 'https://www.healthcare4ppl.com/physician'
