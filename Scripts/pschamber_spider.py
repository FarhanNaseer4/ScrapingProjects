...
class PschamberSpiderSpider(scrapy.Spider):
    name = 'pschamber_spider'
    request_api = 'https://www.pschamber.com/list/searchalpha/{}'
    first_name = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u',
                  'v', 'w', 'x', 'y', 'z']
    custom_settings = {
        'FEED_URI': 'pschamber.csv',
        'FEED_FORMAT': 'csv',
    ...
    def parse(self, response):
        for data in response.css('div.mn-listingcontent'):
            ...
            item['Source_URL'] = 'https://pschamber.com/list/searchalpha/p'
            item['Lead_Source'] = 'pschamber'
