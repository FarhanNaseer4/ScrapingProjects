...
class LouisianabidsSpiderSpider(scrapy.Spider):
    name = 'louisianabids_spider'
    start_urls = ['https://www.louisianabids.us/louisiana-contractors/search.htm']
    base_url = 'https://www.louisianabids.us{}'
    custom_settings = {
        'FEED_URI': 'louisianabids.csv',
        'FEED_FORMAT': 'csv',
    ...
    def detail_page(self, response):
        ...
        fullname = response.xpath('//dt[contains(text(),"Contact")]/following-sibling::dd/text()').get('').strip()
        if fullname:
            item['Full Name'] = fullname
            item['First Name'] = fullname.split(' ')[0].strip()
            item['Last Name'] = fullname.split(' ')[-1].strip()
        item['Detail_Url'] = response.url
        item['Source_URL'] = 'https://www.louisianabids.us/louisiana-contractors/search.htm'
        item['Lead_Source'] = 'Louisianabids'
        item['Occupation'] = 'Government Contractors'
        yield item
