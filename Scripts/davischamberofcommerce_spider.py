...
class DavischamberofcommerceSpiderSpider(scrapy.Spider):
    name = 'davischamberofcommerce_spider'
    start_urls = ['https://business.davischamberofcommerce.com/list/']

    custom_settings = {
        'FEED_URI': 'davischamberofcommerce.csv',
        'FEED_FORMAT': 'csv',
    ...
    def detail_page(self, response):
        item = response.meta['item']
        item['Business Name'] = response.css('div.DC-headertitle h2::text').get('').strip()
        ...
        item['Email'] = response.css('div.gz-member-repname::text').get('').strip()
        ...
        if fullname:
            ...
            if len(fullname.split(' ')) > 2:
                ...
                item['Last Name'] = fullname.split(' ')[-1].strip()
        item['Source_URL'] = 'https://business.davischamberofcommerce.com/list/'
        item['Occupation'] = response.css('span.gz-cat::text').get('').strip()
        item['Lead_Source'] = 'business.davischamberofcommerce'
