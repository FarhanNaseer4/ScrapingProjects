...
class BarringtonchamberSpiderSpider(scrapy.Spider):
    name = 'barringtonchamber_spider'
    start_urls = ['https://business.barringtonchamber.com/list/']

    custom_settings = {
        'FEED_URI': 'barringtonchamber.csv',
        'FEED_FORMAT': 'csv',
    ...
    def parse(self, response):
        for data in response.xpath('//li[@class="gz-subcats-col1"]/a'):
    ...
    def parse_listing(self, response):
        ...
        for data in response.css('div.card-header a'):
            ...
            yield response.follow(url=detail_url, callback=self.detail_page, headers=self.headers, meta={'item': item})
    ...
    def detail_page(self, response):
        ...
        item['Source_URL'] = 'https://business.barringtonchamber.com/list/'
        item['Occupation'] = response.css('span.gz-cat::text').get('').strip()
        item['Lead_Source'] = 'barringtonchamber'
